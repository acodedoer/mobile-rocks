var petsgameover = new Phaser.Class({

    Extends: Phaser.Scene,

    initialize:

    function petsgameover ()
    {
        Phaser.Scene.call(this, { key: 'petsgameover' });
    },

    create: function () { 
        CameraPreview.hide()
        let score = window.localStorage.getItem('petsrandomscore');
        let best = window.localStorage.getItem('petsrandombestscore');
        ...
        new ButtonLink({scene:this,x:window.innerWidth/2,y:window.innerHeight-10, sprite:"button_countdown", link:"petrandom"}).setScale(this.game.global.scaler).setOrigin(0.5,1);

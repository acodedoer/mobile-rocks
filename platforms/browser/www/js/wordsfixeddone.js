var wordsfixeddone = new Phaser.Class({

    Extends: Phaser.Scene,

    initialize:

    function wordsfixeddone ()
    {
        Phaser.Scene.call(this, { key: 'wordsfixeddone' });
    },

    create: function () { 
        CameraPreview.hide()
        let score = window.localStorage.getItem('wordsfixedscore');
        let time = this.toTime(window.localStorage.getItem('wordsfixedtime'));
        let besttime = this.toTime(window.localStorage.getItem('wordsfixedtime'));
        ...
        new ButtonLink({scene:this,x:window.innerWidth/2,y:window.innerHeight-10, sprite:"button_timed", link:"wordsfixed"}).setScale(this.game.global.scaler).setOrigin(0.5,1);

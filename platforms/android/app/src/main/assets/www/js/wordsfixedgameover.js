var wordsfixedgameover = new Phaser.Class({

    Extends: Phaser.Scene,

    initialize:

    function wordsfixedgameover ()
    {
        Phaser.Scene.call(this, { key: 'wordsfixedgameover' });
    },

    create: function () { 
        CameraPreview.hide()
        let score = window.localStorage.getItem('wordsfixedscore');
        let best = window.localStorage.getItem('wordsfixedbestscore');
        ...
        new ButtonLink({scene:this,x:window.innerWidth/2,y:window.innerHeight-10, sprite:"button_countdown", link:"wordsfixed"}).setScale(this.game.global.scaler).setOrigin(0.5,1);
    }
});